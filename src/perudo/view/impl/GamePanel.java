...
import perudo.view.impl.panels.PlayerHandPanel;
...
public class GamePanel extends JPanel {
	...
	private PlayerHandPanel pnlHand;
	...
	public GamePanel() {
		...
		this.pnlHand = null;
		/*
		 * this.pnlTable = null; try { this.pnlTable = new
		 * TablePanel(ImageIO.read(new
		 * File(GamePanel.class.getResource("/images/table.jpg").getPath()))
		 * .getScaledInstance(720, 480, Image.SCALE_DEFAULT)); } catch
	...
	public void setGame(final Game game) {
		...
		if (this.user.isPresent()) {
			if (this.pnlHand == null) {
				this.pnlHand = new PlayerHandPanel(this.game.get(), this.user.get());
			} else {
				this.pnlHand.setGame(game);
			}
		}
		this.add(this.pnlHand, BorderLayout.CENTER);
		if (this.game.get().getTurn().equals(this.user.get())) {
	...
	public void playNotify(Game game, User user) {
		this.setGame(game);
		this.pnlHistory.addInfo(user.getName() + " has play " + game.getCurrentBid().get().getQuantity() + " dices of "
